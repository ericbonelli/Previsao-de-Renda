...
# 📌 Criar as novas features derivadas (Engenharia de Features)
renda_per_capita = 0  # Placeholder, pois a renda real não é conhecida antes da previsão
...
faixa_etaria_Adulto = int(18 <= idade < 50)
faixa_etaria_Idoso = int(idade >= 50)
tempo_emprego_cat_5_10_anos = int(5 <= tempo_emprego < 10)
tempo_emprego_cat_mais_10_anos = int(tempo_emprego >= 10)

# 📌 Mapear categorias para variáveis binárias
tipo_renda_map = {
    "Bolsista": [1, 0, 0, 0],
    "Empresário": [0, 1, 0, 0],
    "Pensionista": [0, 0, 1, 0],
...
# 📌 Criar array de entrada com TODAS as features
entrada = np.array([[
    posse_de_veiculo, posse_de_imovel, qtd_filhos, idade, tempo_emprego, qt_pessoas_residencia,
    ...
    *tipo_residencia_map[tipo_residencia], sexo_M, faixa_etaria_Adulto, faixa_etaria_Idoso,
    ...
    renda_per_capita, idade_tempo_emprego, filhos_tempo_emprego, faixa_etaria_tempo_emprego  # 📌 FEATURES ADICIONADAS
]])
